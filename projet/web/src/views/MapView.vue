...
function buildMarkerTooltip(r: Report): string {
  const parts = [
    ...
    `Statut: ${escapeHtml(r.status)}`,
    r.companyName ? `Entreprise: ${escapeHtml(r.companyName)}` : null,
    `Surface: ${r.surfaceM2 ?? "—"} m²`,
    `Budget: ${r.budgetAmount ?? "—"}`,
    `Avancement: ${r.progressPercent ?? "—"}%`,
    `NEW: ${formatDate(r.statusNewAt)}`,
    `IN_PROGRESS: ${formatDate(r.statusInProgressAt)}`,
    `DONE: ${formatDate(r.statusDoneAt)}`
  ].filter(Boolean);
...
function buildMarkerPopup(r: Report): string {
  const parts = [
    ...
    `Surface: ${r.surfaceM2 ?? "—"} m² · Budget: ${r.budgetAmount ?? "—"} · Avancement: ${r.progressPercent ?? "—"}%`,
