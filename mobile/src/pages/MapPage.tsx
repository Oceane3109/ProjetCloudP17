...
export default function MapPage() {
  ...
  useEffect(() => {
    ...
    for (const r of filtered) {
      ...
      const surface = String((r as any).surfaceM2 ?? "—");
      const budget = String((r as any).budgetAmount ?? "—");
      const level = (r as any).level != null ? String((r as any).level) : "—";
      const progress = Number((r as any).progressPercent ?? 0) || 0;
      const company = String((r as any).companyName ?? "—");
      ...
      m.bindTooltip(
        `<b>${escapeHtml(r.title)}</b><br/>Status: ${escapeHtml(r.status)}<br/>Entreprise: ${company}<br/>Surface: ${surface} m²<br/>Niveau: ${level}<br/>Budget: ${budget}<br/>Avancement: ${progress}%<br/>NEW: ${dNew}<br/>IN_PROGRESS: ${dProg}<br/>DONE: ${dDone}`,
